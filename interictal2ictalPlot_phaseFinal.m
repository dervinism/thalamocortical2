...
for iPair = 1:numel(dataPairs)
    ...
    dataOI_1 = phaseDiffRasterMeanAlignedScaledFinal_i(:,indsOI);
    dataOICI95_1 = phaseDiffRasterCI95AlignedScaledFinal_i(:,indsOI);
    
    %load(dataPairs{iPair}{2})
    indsOI = tStartAlignedScaled >= 0 & tEndAlignedScaled <= 0;
    dataOI_2 = phaseDiffRasterMeanAlignedScaledFinal(:,indsOI);
    dataOICI95_2 = phaseDiffRasterCI95AlignedScaledFinal(:,indsOI);
    ...
    savefig(gcf, 'phaseFinal_evo.fig', 'compact');
    ...
    exportFig(gcf, 'phaseFinal_evo.eps','-depsc','-r1200', paperSize);
end