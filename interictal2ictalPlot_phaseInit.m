...
for iPair = 1:numel(dataPairs)
    ...
    dataOI_1 = phaseDiffRasterMeanAlignedScaledInitial_i(:,indsOI);
    dataOICI95_1 = phaseDiffRasterCI95AlignedScaledInitial_i(:,indsOI);
    
    %load(dataPairs{iPair}{2})
    indsOI = tStartAlignedScaled >= 0 & tEndAlignedScaled <= 0;
    dataOI_2 = phaseDiffRasterMeanAlignedScaledInitial(:,indsOI);
    dataOICI95_2 = phaseDiffRasterCI95AlignedScaledInitial(:,indsOI);
    ...
    savefig(gcf, 'phaseInitial_evo.fig', 'compact');
    ...
    exportFig(gcf, 'phaseInitial_evo.eps','-depsc','-r1200', paperSize);
end