...
dataPairs = {{'frData.mat','frData.mat'}};
margin = 2000;
...
for iPair = 1:numel(dataPairs)
    ...
    dataOI_1 = trMeanAlignedScaled_i(:,indsOI);
    dataOICI95_1 = trCI95AlignedScaled_i(:,indsOI);
    
    %load(dataPairs{iPair}{2})
    indsOI = tStartAlignedScaled >= 0 & tEndAlignedScaled <= 0;
    dataOI_2 = trMeanAlignedScaled(:,indsOI);
    dataOICI95_2 = trCI95AlignedScaled(:,indsOI);
    ...
    for iBorder = 1:numel(tBorders)
        ...
        fullDataWithMargins(:,inds) = movmean(fullDataWithMargins(:,inds),23,2, 'Endpoints','shrink');
        fullDataCI95WithMargins(:,inds) = movmean(fullDataCI95WithMargins(:,inds),23,2, 'Endpoints','shrink');
    end
    ...
    fVM = frAlignPlot2(fullDataWithMargins, fullDataCI95WithMargins, tOnsetCentred, tBorders, colours, lineTypes, yLabel,...
        legendEntries, 'Tonic firing rate');
    
    xLabel = 'Time to SWD onset (s)';
    ...
    yLim = [0 27];
    yTicks = 0:5:25;
    ax1 = axesProperties({}, 1, 'normal', 'off', 'w', 'Arial', 45, 4/3, 2, [0.003 0.003], 'out',...
    ...
    height = (30/55)*22.5-label(2)-margin(2);
    paperSize = resizeFig(gcf, ax1, width, height, label, margin, 0);
    savefig(gcf, 'tr_evo.fig', 'compact');
    ...
    exportFig(gcf, 'tr_evo.eps','-depsc','-r1200', paperSize);
end