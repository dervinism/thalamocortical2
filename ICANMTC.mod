...
PARAMETER {
    ...
    inact   = 1
    tau_hCa_min = 0.1   (ms)        : minimal value of the Ca2+-dependent inactivation time constant
...
ASSIGNED {
    ...
    mVm_inf
    tau_mVm
    tau_hCa (ms)        : Ca2+-dependent inactivation time constant
    q10
}
...
UNITSOFF
PROCEDURE evaluate_fct(cai(mM)) {  LOCAL alpha, alpha_tau
	alpha = beta * (cai/cac)^n                 : sensitivity
    alpha_tau = beta * (cac/cai)^n
	tau_mCa = 250 :10 :1 / (alpha_tau + beta) / q10         : time constant
	mCa_inf = alpha / (alpha + beta)
...
PROCEDURE inactivation(cai(mM)) {LOCAL cac_h
    cac_h = 0.00036
    if (inact) {
        :hCa_inf = 1/(1 + (cai/0.0002)^5)
        hCa_inf = 1/(1 + (cai/cac_h)^20)
    } else {
        hCa_inf = 1
    }
    :tau_hCa = 1000
    tau_hCa = ((1 / 0.00019) / ((cac_h / cai)^20 + 1) / q10) + tau_hCa_min
}
