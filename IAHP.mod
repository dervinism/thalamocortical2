...
PROCEDURE activation(v(mV), cai(mM), cahvai(mM)) {
    LOCAL caInc
    
    caInc = (cai+cahvai) - cainf
    if (caInc < 0) {
    	caInc = 0
    }
	m1_inf = 1 / ((cac1 / caInc)^power1 + 1)
    m2_inf = 1 / ((cac2 / caInc)^power2 + 1)
    :tau_m1 = ((1/beta1) / ((caInc / cac1)^power1 + 1) / phi) + tau_m1_min
    :tau_m2 = ((1/beta2) / ((caInc / cac2)^power2 + 1) / phi) + tau_m2_min
    tau_m1 = tau_m1_min
    tau_m2 = tau_m2_min
}
UNITSON
